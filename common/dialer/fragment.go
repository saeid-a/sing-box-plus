...
func fragmentTLSClientHello(b []byte, sizeRange opt.IntRange) [][]byte {
	...
	if err != nil {
		...
		// select random indices in three parts, 0-sniStart, sniStart-sniEnd, and sniEnd-packetEnd
		preSniExtIdx := selectRandomIndices(sniStartIdx, sizeRange)
		sniExtIdx := selectRandomIndices(sniLen, opt.IntRange{Min: uint64(sniLen / 4), Max: uint64(sniLen / 2)})
		postSniExtIdx := selectRandomIndices(clientHelloLen-(sniStartIdx+sniLen), sizeRange)
		for i := range sniExtIdx {
			sniExtIdx[i] += sniStartIdx
		}
		for i := range postSniExtIdx {
			postSniExtIdx[i] += sniStartIdx + sniLen
		}
		fragmentIndices = append(preSniExtIdx, sniExtIdx...)
